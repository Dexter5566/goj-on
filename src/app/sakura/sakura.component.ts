...
export class BlossomScene {
    ...
    constructor(config: BlossomSceneConfig = myBlossomSceneConfig) {
    ...
    updatePetal(petal: Petal) {
        if (!petal.x) return;
        if (!petal.y) return;
        if (!petal.xSpeedVariation) return;
        if (!petal.ySpeed) return;
        if (!petal.rotation) return;
...
export const myBlossomSceneConfig: BlossomSceneConfig = {
